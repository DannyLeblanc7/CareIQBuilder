(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Get Problem Details API ===');
        ...
        if (missingFields.length > 0) {
            gs.error('CareIQ Get Problem Details: Missing required fields: ' + missingFields.join(', '));
        ...
        if (isDebugEnabled) {
            ...
            gs.info('About to call getProblemDetails method');
        }

        // Check if the method exists
        if (typeof careiqServices.getProblemDetails !== 'function') {
            gs.error('CareIQ Get Problem Details: getProblemDetails method not found in CareIQServices');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
            response.getStreamWriter().writeString('{"error": "getProblemDetails method not found in CareIQServices"}');
            return;
        }

        var responseBody = careiqServices.getProblemDetails(requestData.problemId);
        ...
        try {
            ...
            gs.error('CareIQ Get Problem Details: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            // Successful response
            if (isDebugEnabled) {
                gs.info('=== GET PROBLEM DETAILS SUCCESS ===');
        ...
        var errorMsg = 'Unexpected server error occurred during problem details fetch';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred during problem details fetch';
        }

        gs.error('CareIQ Get Problem Details Script Error: ' + errorMsg);
