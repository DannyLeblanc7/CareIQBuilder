(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Get Intervention Details API ===');
        ...
        var requiredFields = ['interventionId'];
        var missingFields = [];
        ...
        if (missingFields.length > 0) {
            gs.error('CareIQ Get Intervention Details: Missing required fields: ' + missingFields.join(', '));
        ...
        if (isDebugEnabled) {
            ...
            gs.info('About to call getInterventionDetails method');
        }

        // Check if the method exists
        if (typeof careiqServices.getInterventionDetails !== 'function') {
            gs.error('CareIQ Get Intervention Details: getInterventionDetails method not found in CareIQServices');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
            response.getStreamWriter().writeString('{"error": "getInterventionDetails method not found in CareIQServices"}');
            return;
        }

        var responseBody = careiqServices.getInterventionDetails(
        ...
        try {
            ...
            gs.error('CareIQ Get Intervention Details: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            // Successful response
            if (isDebugEnabled) {
                gs.info('=== GET INTERVENTION DETAILS SUCCESS ===');
        ...
        var errorMsg = 'Unexpected server error occurred while fetching intervention details';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred while fetching intervention details';
        }

        gs.error('CareIQ Get Intervention Details Script Error: ' + errorMsg);
