(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Add Intervention API ===');
        ...
        var requiredFields = ['goalId', 'interventionText', 'category', 'guidelineTemplateId'];
        var missingFields = [];
        ...
        if (missingFields.length > 0) {
            gs.error('CareIQ Add Intervention: Missing required fields: ' + missingFields.join(', '));
        ...
        if (isDebugEnabled) {
            ...
            gs.info('interventionText: ' + requestData.interventionText);
        ...
        if (isDebugEnabled) {
            ...
            gs.info('About to call addInterventionToGoal method');
        }

        // Check if the method exists
        if (typeof careiqServices.addInterventionToGoal !== 'function') {
            gs.error('CareIQ Add Intervention: addInterventionToGoal method not found in CareIQServices');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
            response.getStreamWriter().writeString('{"error": "addInterventionToGoal method not found in CareIQServices"}');
            return;
        }

        var responseBody = careiqServices.addInterventionToGoal(
            requestData.goalId,
            requestData.interventionText,
            requestData.category,
            requestData.guidelineTemplateId,
            requestData.tooltip || '',
            requestData.alternative_wording || '',
            requestData.interventionId || null,  // Explicitly pass null for new interventions
            requestData.libraryId || null  // Pass library_id for library interventions
        );
        ...
        try {
            ...
            gs.error('CareIQ Add Intervention: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            var enhancedResponse = {
                ...
                originalRequest: {
                    ...
                    interventionText: requestData.interventionText,
            ...
            if (isDebugEnabled) {
                gs.info('=== ADD INTERVENTION SUCCESS ===');
                gs.info('Goal ID: ' + requestData.goalId);
                gs.info('Intervention Text: ' + requestData.interventionText);
                gs.info('Category: ' + requestData.category);
        ...
        var errorMsg = 'Unexpected server error occurred during intervention creation';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred during intervention creation';
        }

        gs.error('CareIQ Add Intervention Script Error: ' + errorMsg);
