(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Get Goal Details API ===');
        ...
        if (missingFields.length > 0) {
            gs.error('CareIQ Get Goal Details: Missing required fields: ' + missingFields.join(', '));
        ...
        if (isDebugEnabled) {
            ...
            gs.info('About to call getGoalDetails method');
        }

        // Check if the method exists
        if (typeof careiqServices.getGoalDetails !== 'function') {
            gs.error('CareIQ Get Goal Details: getGoalDetails method not found in CareIQServices');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
            response.getStreamWriter().writeString('{"error": "getGoalDetails method not found in CareIQServices"}');
            return;
        }

        var responseBody = careiqServices.getGoalDetails(requestData.goalId);
        ...
        try {
            ...
            gs.error('CareIQ Get Goal Details: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            // Successful response
            if (isDebugEnabled) {
                gs.info('=== GET GOAL DETAILS SUCCESS ===');
        ...
        var errorMsg = 'Unexpected server error occurred during goal details retrieval';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred during goal details retrieval';
        }

        gs.error('CareIQ Get Goal Details Script Error: ' + errorMsg);
