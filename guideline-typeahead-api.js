(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Guideline Typeahead API ===');
        ...
        if (missingFields.length > 0) {
            gs.error('CareIQ Guideline Typeahead: Missing required fields: ' + missingFields.join(', '));
        ...
        if (isDebugEnabled) {
            gs.info('Calling CareIQServices.getGuidelineTypeahead with searchText: ' + requestData.searchText);
        }
        ...
        if (isDebugEnabled) {
            ...
            gs.info('About to call getGuidelineTypeahead method');
        }
        
        // Check if the method exists
        if (typeof careiqServices.getGuidelineTypeahead !== 'function') {
            gs.error('CareIQ Guideline Typeahead: getGuidelineTypeahead method not found in CareIQServices');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
            response.getStreamWriter().writeString('{"error": "getGuidelineTypeahead method not found in CareIQServices"}');
            return;
        }
        
        var responseBody = careiqServices.getGuidelineTypeahead(requestData.searchText);
        ...
        try {
            ...
            gs.error('CareIQ Guideline Typeahead: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            // Successful response
            if (isDebugEnabled) {
                gs.info('=== GUIDELINE TYPEAHEAD SUCCESS ===');
        ...
        var errorMsg = 'Unexpected server error occurred during guideline typeahead';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred during guideline typeahead';
        }
        
        gs.error('CareIQ Guideline Typeahead Script Error: ' + errorMsg);
