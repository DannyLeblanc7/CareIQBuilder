(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Barrier Typeahead API ===');
        ...
        if (missingFields.length > 0) {
            gs.error('CareIQ Barrier Typeahead: Missing required fields: ' + missingFields.join(', '));
        ...
        if (isDebugEnabled) {
            gs.info('Calling CareIQServices.builderGenericTypeahead with type: barrier, searchText: ' + requestData.searchText);
        }
        ...
        if (typeof careiqServices.builderGenericTypeahead !== 'function') {
            gs.error('CareIQ Barrier Typeahead: builderGenericTypeahead method not found in CareIQServices');
        ...
        var responseBody = careiqServices.builderGenericTypeahead('barrier', requestData.searchText);
        ...
        try {
            ...
            gs.error('CareIQ Barrier Typeahead: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            // Successful response
            if (isDebugEnabled) {
                gs.info('=== BARRIER TYPEAHEAD SUCCESS ===');
        ...
        var errorMsg = 'Unexpected server error occurred during barrier typeahead';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred during barrier typeahead';
        }

        gs.error('CareIQ Barrier Typeahead Script Error: ' + errorMsg);
