(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Add Guideline Relationship API ===');
        ...
        if (missingFields.length > 0) {
            gs.error('CareIQ Add Guideline Relationship: Missing required fields: ' + missingFields.join(', '));
        ...
        if (isDebugEnabled) {
            gs.info('Calling CareIQServices.addGuidelineRelationship with answerId: ' + requestData.answerId + ', guidelineId: ' + requestData.guidelineId);
        }
        ...
        if (isDebugEnabled) {
            ...
            gs.info('About to call addGuidelineRelationship method');
        }
        
        // Check if the method exists
        if (typeof careiqServices.addGuidelineRelationship !== 'function') {
            gs.error('CareIQ Add Guideline Relationship: addGuidelineRelationship method not found in CareIQServices');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
            response.getStreamWriter().writeString('{"error": "addGuidelineRelationship method not found in CareIQServices"}');
            return;
        }
        
        var responseBody = careiqServices.addGuidelineRelationship(requestData.answerId, requestData.guidelineId);
        ...
        try {
            ...
            // If response is empty (204 success), that's expected
            if (!responseBody || responseBody.trim() === '') {
                if (isDebugEnabled) {
                    gs.info('Empty response received - assuming 204 success');
                }
            ...
            gs.error('CareIQ Add Guideline Relationship: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            // Successful response (should be 204)
            if (isDebugEnabled) {
                gs.info('=== ADD GUIDELINE RELATIONSHIP SUCCESS ===');
        ...
        var errorMsg = 'Unexpected server error occurred during add guideline relationship';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred during add guideline relationship';
        }
        
        gs.error('CareIQ Add Guideline Relationship Script Error: ' + errorMsg);
