(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Get Scoring Models API ===');
        ...
        var requiredFields = ['guideline_template_id'];
        var missingFields = [];
        ...
        if (missingFields.length > 0) {
            gs.error('CareIQ Get Scoring Models: Missing required fields: ' + missingFields.join(', '));
        ...
        if (isDebugEnabled) {
            ...
            gs.info('About to call getScoringModels method');
        }

        // Check if the method exists
        if (typeof careiqServices.getScoringModels !== 'function') {
            gs.error('CareIQ Get Scoring Models: getScoringModels method not found in CareIQServices');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
            response.getStreamWriter().writeString('{"error": "getScoringModels method not found in CareIQServices"}');
            return;
        }

        var responseBody = careiqServices.getScoringModels(
        ...
        try {
            ...
            gs.error('CareIQ Get Scoring Models: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            if (isDebugEnabled) {
                gs.info('=== GET SCORING MODELS SUCCESS ===');
                gs.info('Guideline Template ID: ' + requestData.guideline_template_id);
                gs.info('Models found: ' + (parsedResponse.scoring_quantity || 0));
        ...
        var errorMsg = 'Unexpected server error occurred during scoring models fetch';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred during scoring models fetch';
        }

        gs.error('CareIQ Get Scoring Models Script Error: ' + errorMsg);
