(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Add Barrier Relationship API ===');
        ...
        var requiredFields = ['answerId', 'barrierName', 'guidelineTemplateId'];
        var missingFields = [];
        ...
        if (missingFields.length > 0) {
            gs.error('CareIQ Add Barrier Relationship: Missing required fields: ' + missingFields.join(', '));
        ...
        if (isDebugEnabled) {
            ...
            gs.info('barrierName: ' + requestData.barrierName);
            gs.info('barrierId: ' + requestData.barrierId);
        ...
        if (isDebugEnabled) {
            ...
            gs.info('About to call addBarrierRelationship method');
        }

        // Check if the method exists
        if (typeof careiqServices.addBarrierRelationship !== 'function') {
            gs.error('CareIQ Add Barrier Relationship: addBarrierRelationship method not found in CareIQServices');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
            response.getStreamWriter().writeString('{"error": "addBarrierRelationship method not found in CareIQServices"}');
            return;
        }

        var responseBody = careiqServices.addBarrierRelationship(
            requestData.answerId,
            requestData.barrierName,
            requestData.barrierId || null,  // Explicitly pass null for new barriers
        ...
        try {
            ...
            gs.error('CareIQ Add Barrier Relationship: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            var enhancedResponse = {
                ...
                originalRequest: {
                    ...
                    barrierName: requestData.barrierName,
                    barrierId: requestData.barrierId,
                    sortOrder: requestData.sortOrder
                }
            };

            if (isDebugEnabled) {
                gs.info('=== ADD BARRIER RELATIONSHIP SUCCESS ===');
                gs.info('Answer ID: ' + requestData.answerId);
                gs.info('Barrier Name: ' + requestData.barrierName);
        ...
        var errorMsg = 'Unexpected server error occurred during barrier relationship addition';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred during barrier relationship addition';
        }

        gs.error('CareIQ Add Barrier Relationship Script Error: ' + errorMsg);
