(function process(request, response) {
    try {
        ...
        if (isDebugEnabled) {
            gs.info('=== CareIQ Create Problem Bundle API ===');
        ...
        if (!requestData.contentId) {
            gs.error('CareIQ Create Problem Bundle: Missing required field: contentId');
        ...
        if (isDebugEnabled) {
            gs.info('Calling CareIQServices.createProblemBundle with contentId: ' + requestData.contentId);
        }
        ...
        if (typeof careiqServices.createProblemBundle !== 'function') {
            gs.error('CareIQ Create Problem Bundle: createProblemBundle method not found in CareIQServices');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
            response.getStreamWriter().writeString('{"error": "createProblemBundle method not found in CareIQServices"}');
            return;
        }

        var responseBody = careiqServices.createProblemBundle(requestData.contentId);
        ...
        try {
            ...
            gs.error('CareIQ Create Problem Bundle: Invalid JSON response from CareIQ Services');
            response.setStatus(500);
            response.setHeader('Content-Type', 'application/json');
        ...
        if (parsedResponse && parsedResponse.error) {
            ...
            // Successful response
            if (isDebugEnabled) {
                gs.info('=== CREATE PROBLEM BUNDLE SUCCESS ===');
        ...
        var errorMsg = 'Unexpected server error occurred during problem bundle creation';
        try {
            if (e && typeof e.toString === 'function') {
                errorMsg = e.toString();
            }
        } catch (innerE) {
            errorMsg = 'Server error occurred during problem bundle creation';
        }

        gs.error('CareIQ Create Problem Bundle Script Error: ' + errorMsg);
